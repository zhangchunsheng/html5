...
(function(){
	window.JSON = {
		 ...
		 * @param {function} reviver  [option]解析时使用的过滤器
		 * @return {object | array} 返回反序列化后的对象或数组
		 ...
		 * @static
		 * @method stringify
		 * @param {object|array} value 需要进行序列化的对象或者数组
		 * @param {function|array} replace [option]序列化过程中使用的过滤器，如果是数组则数组的每个成员都必须是字符串，表示value中哪些属性是需要序列化的
		 * @param {string|number} spacer [option]用于格式化输出结果，如果是数字则表示缩进的空格个数，如果是字符串则表示用于缩进填充的字符串
