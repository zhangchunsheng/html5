...
function Sidebar() {
	window.Sidebar = function(t) {
		...
		this.icons = new Sheet(embed("i/g/icons.png"), 50, 50), Container.call(this);
		var r = this;
		this.showing = !1;
		...
		this.addMenuToggle = function(e, n, o, s) {
			...
			var d = i(new Sprite(embed("i/g/toggle_background.png")), 388, 10);
			h.addChild(d), d.interactive = !0, d.buttonMode = !0, d.defaultCursor = "pointer";
			var u = i(new Sprite(embed("i/g/toggle_outline.png")), 386, 8);
			h.addChild(u);
			var p = i(new Sprite(embed("i/g/toggle_switch.png")), 427, 12);
			h.addChild(p);
...
function game() {
	...
	function o(t, e) {
		...
		for (var n = 1e3, o = [], s = [], a = 0; a < r.length; a++)
			for (var h = r[a], l = 0; l < h.length; l++) {
				var c = r[a][l];
				if (1 === c) {
					var d = new Sprite(embed("i/g/shadow.png"));
					d.anchor.set(.5, .5), d.frame = 2, this.innerContainer.addChildAt(d, 0), d.alpha = 0;
					var u = new Sheet(embed("i/g/tile.png"), 140, 155);
	...
	var P = Sound.get("i/s/pickup.mp3", 1),
		m = Sound.get("i/s/place.mp3", 1),
		x = Sound.get("i/s/placewrong.mp3", 1),
		b = [Sound.get("i/s/row1.mp3", .8), Sound.get("i/s/row2.mp3", .8), Sound.get("i/s/row3.mp3", .8), Sound.get("i/s/row4.mp3", .8), Sound.get("i/s/row5.mp3", .8), Sound.get("i/s/row6.mp3", .8), Sound.get("i/s/row7.mp3", .8), Sound.get("i/s/row8.mp3", .8), Sound.get("i/s/row9.mp3", .8), Sound.get("i/s/row10.mp3", .8)],
		T = Sound.get("i/s/gameover.mp3", .9),
		X = Sound.get("i/s/highscore.mp3", 1),
		S = Sound.get("i/s/ding.mp3", .8);
	window.clickSound = Sound.get("i/s/click.mp3", .8);
	var C = [
			[Sound.get("i/s/voice_tier1_beautiful.mp3", .8), "漂亮！"],
			[Sound.get("i/s/voice_tier1_goodjob.mp3", .8), "渐入佳境!"],
			[Sound.get("i/s/voice_tier2_gettinggood.mp3", .8), "真酷!"],
			[Sound.get("i/s/voice_tier2_ohyeah.mp3", .8), "哦耶！"],
			[Sound.get("i/s/voice_tier3_gorgeous.mp3", .8), "大神！"],
			[Sound.get("i/s/voice_tier3_terrific.mp3", .8), "叼叼叼！"],
			[Sound.get("i/s/voice_tier4_genius.mp3", .8), "收下我的膝盖！"],
			[Sound.get("i/s/voice_tier4_stunning.mp3", .8), "开挂了吧！"],
			[Sound.get("i/s/voice_tier5_fantastic.mp3", .8), "了不起!"],
			[Sound.get("i/s/voice_tier5_flabbergasting.mp3", .8), "智商突破天际！"],
			[Sound.get("i/s/voice_tier6_astounding.mp3", .8), "无人能敌!"],
			[Sound.get("i/s/voice_tier6_explosive.mp3", .8), "爆炸！"],
			[Sound.get("i/s/voice_tier7_incredible.mp3", .8), "奇迹!"],
			[Sound.get("i/s/voice_tier7_mindblowing.mp3", .8), "超神!"]
		],
		M = Music.get("i/s/music.mp3", 1),
		_ = Music.get("i/s/menu.mp3", 1);
	...
	var E = "score.v1",
		...
		L = .2,
		B = function() {
			ModalOverlayContent.call(this), this.addHeadline("游戏结束"), this.addLead("没有地方放了！"), this.innerHeight = 250, this.blurClose = !1
		};
	...
	var O = function() {
		...
		ModalOverlayContent.call(this), this.addHeadline("游戏结束"), this.innerHeight = 850, this.blurClose = !1, i(H);
		...
		switch(true){
			case H>= 200000:
				...
				break;
			case H>= 100000:
				myText = '这个成绩可以完爆全世界99%的人！\n晒成绩再完爆剩下的1%！';
				break;
			case H>= 50000:
				myText = '你的智商已突破天际！\n请加入68微游戏门下吧！';
				break;
			case H>= 20000:
				myText = '这么高的智商，让别人怎么活？\n必须要晒下成绩啊！';
				break;
			case H>= 10000:
				myText = '吾之友屌似汝,而今坟头草仗高！';
				break;
			case H>= 9000:
				myText = '大神，请收下我的膝盖！\n晒成绩收集更多膝盖！';
				break;
		...
		var encourageText = this.addTextBlock(myText, 50,200,'blue');
		encourageText.y = 400;
		var shareButton = this.addSocialButton('晒成绩','这个分享，我给满分！',function(){
			play68_submitScore(H);
		},4675430);
	...
	for (var q, Q = new Container, J = 150, Z = J / 2, $ = 133, tt = [], et = 9, it = 4, rt = 0; et > rt; rt++) {
		...
		for (var nt = Math.abs(it - rt), ot = 0; et - nt > ot; ot++) {
			var st = new Sheet(embed("i/g/tile.png"), 140, 155);
	...
	window.showGameOver = function() {
		...
		Modal.show(new RateGameModal(embed("i/g/star.png"), 9014527))
...
function preloader() {
	...
	var r = new Sprite(embed("i/g/frvrlogo.png"));
	r.ratio = 1;
...
var ModalOverlayContent = function() {
	...
	}, this.addTextBlock = function(t, e, i, c) {
		var r = new Text(t, i || 200, e || 90, c || "#2c2c2c", '"Helvetica Neue","Trebuchet MS", Helvetica, sans-serif',!1,'center');
	...
	this.addButton = function(t, e, i) {
		...
		return r.y = 650, this.addChild(r)
		...
		return n.y = 519, this.addChild(n)
